...
const MAX_INITDATA_WAIT_MS = 5000;
const MAX_INITDATA_POLL_MS = 120;

async function waitForMaxInitDataRaw(timeoutMs = MAX_INITDATA_WAIT_MS): Promise<string> {
  const stopAt = Date.now() + timeoutMs;
  while (Date.now() < stopAt) {
    prepareMaxWebApp();
    const raw = getMaxInitDataRaw();
    if (raw) return raw;
    await new Promise((resolve) => setTimeout(resolve, MAX_INITDATA_POLL_MS));
  }
  return '';
}
...
export default function MaxAuthScreen() {
  ...
  const botUsername = process.env.EXPO_PUBLIC_MAX_BOT_USERNAME || '';

  const [flowState, setFlowState] = useState<FlowState>('checking');
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [notice, setNotice] = useState<string | null>(null);
  const [maxSessionToken, setMaxSessionToken] = useState('');
  ...
  const finalizeSignIn = useCallback(
    async (token: string) => {
      const data = await maxSignIn(token);
  ...
  const checkContactStatusByToken = useCallback(
    async (token: string) => {
      if (!token) return;
      const status = await maxContactStatus(token);
    ...
    async (
      ...
      try {
        ...
        if (result.ok && result.phoneE164) {
          const contactRes = await maxContact(token, result.phoneE164);
        ...
        if (!result.ok && source === 'manual') {
          setNotice('MAX не передал номер. Повторите запрос или откройте бота для ручной отправки контакта.');
        }
  ...
  const bootstrap = useCallback(async () => {
    ...
    try {
      ...
      if (!isMaxMiniAppLaunch()) {
        setFlowState('need_phone');
        setError('Откройте эту страницу из MAX Mini App.');
        return;
      }
      const initDataRaw = await waitForMaxInitDataRaw();
      if (!initDataRaw) {
        setFlowState('need_phone');
        setError('Не удалось получить MAX initData. Проверьте, что Mini App открыта внутри MAX, и нажмите обновить.');
        return;
      }
      const initData = await maxInit(initDataRaw);
      setMaxSessionToken(initData.maxSessionToken);

      if (initData.state === 'NEED_PHONE') {
        setFlowState('need_phone');
        await requestContactAndSync(initData.maxSessionToken, 'auto', false);
      } else {
        await applyState(initData.state, initData.maxSessionToken, initData.conflictUserHint ?? null);
      }
    } catch (e: any) {
      setFlowState('need_phone');
      setError(e?.message || 'Не удалось инициализировать вход через MAX');
    } finally {
      setBusy(false);
  ...
  useEffect(() => {
    ...
    void bootstrap();
    // deliberately once-on-mount to avoid bootstrap loops when hook dependencies
    // (e.g. router/context setters) are recreated by parent renders.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const handleRequestContact = useCallback(async () => {
    if (!maxSessionToken) return;
    setNotice('Запрашиваем номер телефона в MAX...');
    await requestContactAndSync(maxSessionToken, 'manual', true);
  }, [maxSessionToken, requestContactAndSync]);

  const openBot = useCallback(() => {
    if (!botUsername) return;
    void Linking.openURL(`https://max.ru/${botUsername}`);
  }, [botUsername]);
  ...
  const handleLink = useCallback(async () => {
    ...
    try {
      ...
      setError(e?.message || 'Не удалось привязать MAX к аккаунту');
    } finally {
      setBusy(false);
    }
  }, [email, maxSessionToken, password, router, setAuthenticated, setProfile]);
        ...
        <ScrollView contentContainerStyle={styles.container}>
          <View style={styles.card}>
            <View style={styles.hero}>
              <Text style={styles.heroBadge}>MAX Mini App</Text>
              <Text style={styles.title}>Вход через MAX</Text>
            ...
            {flowState === 'need_link' ? (
              <View style={styles.section}>
                ...
                <Text style={styles.sectionText}>
                  Войдите по email/паролю, чтобы привязать MAX.
                </Text>
