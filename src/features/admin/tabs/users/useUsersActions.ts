...
function toEditorState(profile: Profile): UsersEditorState {
  return {
    ...
    onecPhysicalPersonGuid: profile.employeeProfile?.onecPhysicalPersonGuid ?? null,
  ...
  const saveEditor = useCallback(async () => {
    ...
    try {
      ...
      if (editor.hasEmployeeProfile) {
        ...
        if ((editor.onecPhysicalPersonGuid ?? null) !== (editorInitial.onecPhysicalPersonGuid ?? null)) {
          employeePatch.onecPhysicalPersonGuid = editor.onecPhysicalPersonGuid;
        }
        ...
        if (Object.prototype.hasOwnProperty.call(employeePatch, 'onecPhysicalPersonGuid')) {
          userPatch.onecPhysicalPersonGuid = employeePatch.onecPhysicalPersonGuid;
          delete employeePatch.onecPhysicalPersonGuid;
        }
        if (Object.keys(userPatch).length) {
          await adminUpdateUser(editorUserId, userPatch);
          hasChanges = true;
