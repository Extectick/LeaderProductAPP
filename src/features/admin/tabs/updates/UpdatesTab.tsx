...
import { ActivityIndicator, Button, Dialog, Portal, Switch, Text, TextInput, TouchableRipple } from 'react-native-paper';
...
export default function UpdatesTab({ active, styles, colors, isWide }: UpdatesTabProps) {
  ...
  return (
    <>
      ...
      <Portal>
        <Dialog visible={!!infoModal} onDismiss={closeInfo}>
          <Dialog.Title>{infoModal?.title || ''}</Dialog.Title>
          <Dialog.Content>
            <Text variant="bodyMedium">{infoModal?.message || ''}</Text>
          </Dialog.Content>
          <Dialog.Actions>
            <Button onPress={closeInfo}>Закрыть</Button>
          </Dialog.Actions>
        </Dialog>
      </Portal>
    </>
